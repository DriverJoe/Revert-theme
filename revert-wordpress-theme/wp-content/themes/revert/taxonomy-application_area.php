 ...
 *
 * Uses the same product grid layout as the main products archive.
 *
 * @package ReVert
 * @since 1.0.0
 */
...
<section class="py-16 bg-muted">
    <div class="container">
        <div class="max-w-3xl mx-auto text-center mb-12">
            <h1 class="text-4xl md:text-5xl font-bold mb-4"><?php echo esc_html($current_term->name); ?></h1>
            <?php if ($current_term->description) : ?>
                <p class="text-xl text-muted-foreground"><?php echo esc_html($current_term->description); ?></p>
            <?php else : ?>
                <p class="text-xl text-muted-foreground">Innovative agricultural solutions for sustainable farming</p>
            <?php endif; ?>
        </div>
        ...
        ?>
            <div class="flex flex-wrap justify-center gap-2 mb-12">
                <a href="<?php echo get_post_type_archive_link('revert_product'); ?>"
                   class="inline-flex items-center justify-center h-9 px-4 rounded-md border bg-background hover:bg-accent hover:text-accent-foreground">
                    All Products
                </a>
